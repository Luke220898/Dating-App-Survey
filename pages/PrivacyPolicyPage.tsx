...
const PrivacyPolicyPage: React.FC = () => {
    ...
    return (
        <div className="bg-white min-h-screen cv-auto">
            ...
            <main className="p-4 sm:p-8">
                <div className="max-w-3xl mx-auto text-gray-700 leading-relaxed">
                    ...
                    <section className="mb-8">
                        ...
                        <p dangerouslySetInnerHTML={{ __html: sanitize(t('privacy.legalBasis.text') as string) }} />
                    </section>
                    ...
                    <section className="mb-8">
                        ...
                        <ul className="list-disc list-inside mt-2 space-y-1">
                            {(t('privacy.processing.items') as string[]).map((item, i) => <li key={i} dangerouslySetInnerHTML={{ __html: sanitize(item) }} />)}
                        </ul>
                        ...
                        <p className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded-lg text-yellow-900"
                            dangerouslySetInnerHTML={{ __html: sanitize(t('privacy.processing.note') as string) }}
                        />
                    </section>
                    ...
                    <section className="mb-8">
                        ...
                        <ul className="list-disc list-inside mt-2 space-y-1">
                            {(t('privacy.rights.items') as string[]).map((item, i) => <li key={i} dangerouslySetInnerHTML={{ __html: sanitize(item) }} />)}
                        </ul>
                        <p className="mt-2" dangerouslySetInnerHTML={{ __html: sanitize(t('privacy.rights.footer') as string) }} />
                    </section>
                </div>
            </main>
        </div>
